...
## 실시간 이슈 봇 (신규, 개발 예정)
- 목적: DART 공시 + RSS 이벤트 → 메리츠 Tech 채널 스타일 요약 → 관리자 승인 → @noderesearch 발송
- 메리츠 Tech 채널(t.me/merITz_tech) 70+ 샘플 학습 기반, 전 33개 섹터 커버
- 상세 스펙: **ISSUE_BOT_SPEC.md** 참조
- 상태: 설계 완료, Phase 0 착수 대기
- 기존 브리핑과 독립 모듈 (동일 봇/채널 공유, 시간·로직 충돌 방지 설계 포함)
