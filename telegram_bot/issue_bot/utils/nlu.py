...
# ===== 한국어 자연어 노이즈 사전 (광범위) =====
# 카테고리별로 미리 광범위하게 등록 → 매번 패치 부담 ↓
# 회사명·산업명·기술명 같은 핵심 키워드는 영향 없음 (이런 표현은 회사명에 안 들어감).

_NOISE_PHRASES = {
    "요청_동사": [
        "검색해줘", "검색해봐", "검색해주세요", "검색해", "검색",
        "찾아줘", "찾아봐", "찾아주세요", "찾아", "찾기",
        "뽑아줘", "뽑아봐", "뽑아",
        "보여줘", "보여봐", "보여주세요", "보여",
        "들려줘", "말해줘", "알려줘봐", "알려주세요", "알려줘", "알려",
        "체크해줘", "체크해봐", "체크해", "체크",
        "확인해줘", "확인해봐", "확인해", "확인",
        "올려줘", "올려봐", "올려",
        "줘봐", "받아와", "받아봐", "받아", "가져와봐", "가져와",
        "정리해줘", "정리해봐", "정리해", "정리",
        "요약해줘", "요약해봐", "요약해", "요약",
        "보내줘", "보내봐",
    ],
    "존댓말_종결": [
        "있나요", "있습니까", "있을까요", "있는가요", "있는지요",
        "해주실래요", "해주시면", "해주실 수 있나요",
        "부탁드려요", "부탁드립니다", "부탁드릴게요",
        "있어요", "어떨까요", "어떤가요", "괜찮을까요",
        "주실래요", "주실 수 있나요",
    ],
    "반말_종결": [
        "있냐", "있어", "있나", "있는지", "있을까", "있을지", "있는데", "있을까봐",
        "뭐있어", "뭐나왔냐", "뭐나왔어", "뭐가있어",
        "어때", "어떨까", "어떤지", "어떨지",
    ],
    "모호_형용사": [
        "주요한", "주요", "중요한", "중요", "핵심적인", "핵심", "주된", "특별한",
        "굵직한", "대단한", "엄청난", "강력한", "강한", "센", "큼직한",
        "파급력있는", "파급력", "파급",
        "영향력있는", "영향력", "영향",
        "핫한", "뜨거운", "주목할만한", "주목할", "주목",
    ],
    "모호_명사": [
        "이슈들", "이슈나", "이슈가", "이슈는", "이슈도", "이슈",
        "것들", "것은", "것이", "거들", "거가", "거", "건들", "건이", "건",
        "내용", "내역", "현황", "결과", "상황", "정보", "자료",
        "소식들", "소식",
        "사건", "사건들",
    ],
    "시간_부사": [
        "방금전", "방금", "조금전", "막", "아까",
        "이제는", "이제껏", "이제",
        "지금까지", "지금", "현재까지", "현재는", "현재", "여태껏", "여태",
    ],
    "빈도_정도": [
        "자주", "많이", "거의", "대체로", "보통", "흔히",
        "약간", "조금", "다소", "꽤", "되게", "상당히", "특히",
    ],
    "추측_표현": [
        "라고하는", "라는데", "라는", "라고",
        "한대요", "한대", "라네", "라네요",
    ...
    "분기_복수표현": [
        # "4개 분기" 같은 복수 분기 표현 — 향후 4분기 카드 트리거 (현재는 클렌징)
        "4개 분기", "4개분기", "3개 분기", "3개분기",
        "2개 분기", "2개분기", "1개 분기", "1개분기",
        "분기 추이", "분기추이",
        "각 분기", "각각 분기", "각각의 분기",
    ],
}

# 광범위 클렌징 정규식 — 긴 패턴 우선 매칭 (조사 잘림 방지)
_ALL_NOISE = sorted(
    {p for cat in _NOISE_PHRASES.values() for p in cat},
    key=len, reverse=True,
)
_NOISE_PATTERN = re.compile(
    "|".join(re.escape(p) for p in _ALL_NOISE)
)


def _clean_korean_noise(text: str) -> str:
    """광범위 한국어 노이즈 표현 제거."""
    if not text:
        return text
    return _NOISE_PATTERN.sub(" ", text)
...
def _rule_nlu(text: str) -> str:
    ...
    # 보고서 종류 키워드 (실적·자사주·M&A 등) → 자동 dart 의도
    if not is_news and not is_dart:
        rk, _ = extract_report_keyword(text)
        if rk:
            is_dart = True

    # 분기 패턴 (1Q26·1분기) → 자동 dart 의도 (공시 관련)
    if not is_news and not is_dart:
    ...
    # 4-6. 광범위 한국어 노이즈 사전 적용 (사용자 자유 표현 대응)
    remaining = _clean_korean_noise(remaining)
    ...
    remaining = re.sub(
        ...
        # 정성 형용사 — 검색어로 의미 X
        r"파급력있는|파급력|파급|영향력|영향|"
        r"강력한|강한|큰|굵직한|중대한|핫한|"
        r"있는|있을|있냐|있는지|있을까|"
        # 의미 약한 명사/관용 표현
