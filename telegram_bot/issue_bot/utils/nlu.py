...
def _rule_nlu(text: str) -> str:
    ...
    remaining = re.sub(
        ...
        # 동사·형용사 활용
        r"있었어|있었나|있었던|있었을|있을까|있을지|있나|있는|있던|있어|있을|있는지|"
        r"나왔어|나왔나|나왔던|나왔는지|나온|나왔을|나오는|"
        r"올라왔어|올라왔나|올라왔던|올라왔는지|올라온|올라온지|"
        r"발생한|발생했어|발생했나|"
        # 의문사·부사
        r"무엇이|무엇을|뭐가|뭐를|뭐|무슨|어떤|어떻게|얼마나|"
        r"좀|혹시나|혹시|한번|해봐|해줘|줘봐|"
        r"알려줘|보여줘|확인|체크|찾아줘|봐줘|보내줘|"
        # 의미 약한 명사/관용 표현
        r"공시|뉴스|기사|헤드라인|소식|보도|"
        r"사이에|사이의|사이|동안|중에|중에서|중|"
