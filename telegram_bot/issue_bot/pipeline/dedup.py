...
# 정식 기업명 → 종목코드/ticker 정규화.
# DART 공시 이벤트는 ticker(6자리 종목코드) 보유, RSS 뉴스는 제목에서 별칭 추출만 가능.
# 둘의 dedup_key prefix가 달라지면 cluster 매칭 실패 → 같은 이벤트가 중복 카드로
# 발송되는 버그 발생. 이 테이블로 RSS에서 추출된 canonical 이름을 ticker로 변환,
# DART와 동일한 prefix 생성 → cluster 자동 묶임 + "공시 우선 dedup" 정책 작동.
#
# 2026-04-24 기아 누락 사례 계기. 대형 상장사 위주로 점진적 확대.
_CANONICAL_TO_TICKER = {
    # 반도체·IT (국내)
    "SK하이닉스": "000660",
    "삼성전자": "005930",
    # 2차전지·EV (국내)
    "LG에너지솔루션": "373220",
    "삼성SDI": "006400",
    "에코프로": "086520",  # 에코프로비엠(247540)도 있으나 대부분 "에코프로"로 통칭
    "포스코퓨처엠": "003670",
    "현대차": "005380",
    "기아": "000270",
    # 디스플레이·부품 (국내)
...
_COMPANY_ALIASES = {
    ...
    "카카오": ["카카오"],
    # 금융·지주 (2026-04-24 추가 — 잠정실적 시즌 누락 계기)
    "하나금융지주": ["하나금융지주", "하나금융", "하나지주"],
    "신한지주": ["신한지주", "신한금융지주", "신한금융"],
    "KB금융": ["KB금융", "KB금융지주", "국민은행"],
    # 현대차 그룹 계열 (2026-04-24 추가)
    "현대모비스": ["현대모비스", "모비스"],
    "현대글로비스": ["현대글로비스", "글로비스"],
    "현대제철": ["현대제철"],
    "현대위아": ["현대위아"],
    # 삼성 그룹 계열 (2026-04-24 추가)
    "삼성E&A": ["삼성E&A", "삼성이앤에이", "삼성엔지니어링"],
    "삼성SDS": ["삼성SDS", "삼성에스디에스"],
    # 기타 잠정실적 상위 (2026-04-24 추가)
    "토비스": ["토비스"],
}
...
def generate_dedup_key(event: dict) -> str:
    ...
    if ticker == "NONE":
        ...
        if extracted:
            # canonical 이름을 종목코드/ticker로 정규화 → DART 공시와 같은 prefix 형성
            # (예: "기아" RSS → "000270" / DART 기아 공시도 "000270" → cluster 매칭)
            mapped = _CANONICAL_TO_TICKER.get(extracted)
            ticker = mapped if mapped else f"C_{extracted}"
        else:
            # fallback: 회사명 앞 8자
