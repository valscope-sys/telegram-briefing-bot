...
- HIGH: 밸류체인·사이클·구조 변화 중 하나 강함 (Peer 실적, 가격 인상, Capex 1000억+, 5% 공시, BW/CB 발행, **고품질 리서치 메가 인사이트**)
- NORMAL: 의미는 있으나 즉시성 낮음 (월별 수출통계 단순 발표, 마이너 매체 일반 산업 동향)
- SKIP: 시사점 없거나 노이즈

⚠️ **고품질 리서치 리포트는 NORMAL 아닌 HIGH로 처리**. 다음 [고품질 매체 가점] 섹션 참조.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━
[⭐ 고품질 매체 가점 — 자동 HIGH 후보 (2026-04-29 추가)]

source/매체가 아래에 해당하면 **NORMAL → HIGH 강제 가점**.
이런 매체는 이미 1차 큐레이션을 거친 메가 인사이트라 우리가 다시 SKIP 내리면 안 됨.

**리서치 기관 (반드시 HIGH 이상)**:
- TrendForce, Counterpoint, Omdia, IDC, Gartner, Prismark, SemiAnalysis
- 산업 사이클 시그널·BB ratio·가동률·가격 동향 = 한국 밸류체인 직격

**반도체·테크 전문지 (HIGH 후보)**:
- Digitimes, Tom's Hardware, AnandTech, ServeTheHome, EE Times
- 부품·장비·기술 1차 보도

**광역 1차 외신 (HIGH 후보, 단 일반 정치 잡음 제외)**:
- Reuters Tech, Bloomberg Tech, Nikkei Asia, FT, WSJ Tech
- 분석가 단독: Ming-Chi Kuo

**국내 1차 매체 (HIGH 후보)**:
- 전자신문, 디지털타임스, ZDNet Korea, Business Post

판정 흐름:
1. source가 위 매체 + 제목/본문에 산업·기업·기술·가격 키워드 → HIGH 자동
2. 단, 같은 매체라도 "중국 경제 일반" / "미국 일반 정치" / "사회면" 기사면 평소 기준
3. 본문 짧아도 (RSS summary만) 제목 기준으로 판단 가능

━━━━━━━━━━━━━━━━━━━━━━━━━━━━
[⚠️ 실적 공시 — "기업·규모 기반 선별" 정책 (2026-04-29 개정)]

**중요 변경**: 이전에 "잠정실적 = 무조건 URGENT" 규칙이 있었으나 무차별 카드 폭증 문제로 폐기.
이제 잠정실적/30%변경 공시는 반드시 본문·기업·규모 평가 후 판정.

1) **국내 상장사 분기 실적 공시** (잠정실적 / 손익구조 변경):
   본문·기업명을 보고 다음 기준으로 판정:

   | 조건 | 판정 |
   |---|---|
   | 시총 상위 (KOSPI200 / KOSDAQ150 추정 종목) | URGENT |
   | 핵심 Peer 기업 (반도체·2차전지·바이오·자동차 대형주) | URGENT |
   | 실적 서프라이즈 — YoY/QoQ ±30% 이상 변동 | URGENT |
   | 적자전환 / 흑자전환 | URGENT |
   | 중형주 (시총 5000억~1조 추정) + 핵심 부품주 | HIGH |
   | 손익구조 30%/15% 변경 (변동률 50%+) | URGENT |
   | 손익구조 변경 (변동률 30~50%) | HIGH |
   | 그 외 소형주 일반 잠정실적 | NORMAL (카드 발송 X) |
   | 본문에 수치 없는 단순 "실적 발표 예정" | SKIP |

   ⚠️ **소형주 무차별 자동 카드 금지** — 시총·관심도 보고 신중 판정.
   기업명을 모를 경우 "중견 기업" 가정으로 NORMAL이 안전.
