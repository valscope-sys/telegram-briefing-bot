...
[레퍼런스 — 이 수준의 깊이와 흐름으로 작성하세요]
"트럼프의 이란 2주 휴전 선언과 삼성전자 57.2조원 잠정실적이 겹치면서, 3월 이후 이어져온 조정 장세의 분위기가 반전된 하루였습니다.

전쟁과 실적 두 축이 동시에 긍정적으로 전환된 것은 이번이 처음이며, 한 달 넘게 변동성에 시달려온 시장에 안도감이 확산되고 있습니다.

코스피는 휴전 기대감과 삼성전자 호실적에 힘입어 장 초반부터 강한 상승세를 이어갔습니다. WTI가 17% 급락하면서 에너지 비용 부담이 크게 완화됐고, 이는 인플레이션 우려 축소와 금리 인하 기대감 회복으로 연결되면서 성장주 밸류에이션 부담을 덜어주는 구조로 작용했습니다.

반도체 섹터에서 삼성전자와 SK하이닉스가 동반 급등하며 업종 상승을 이끌었습니다. 삼성전자는 컨센서스 38조원을 50% 상회하는 57.2조원의 분기 잠정실적을 발표하며 메모리 업황의 건재함을 확인시켰습니다.

다만 이번 휴전이 2주 한시적이라는 점에서 에너지 비용 부담이 재차 부각될 가능성은 추후 확인이 필요한 부분이며, 이런 고민이 생기는 것은 한 달간 변동성을 겪어온 투자자들의 자연스러운 반응이라고 생각합니다."

위 레퍼런스의 핵심 패턴:
- 첫 문장에 "국면 정의" (조정 장세의 분위기가 반전)
- 인과관계 체인 3단계 이상 (유가 급락→인플레 완화→금리→밸류)
- "3월 이후" 같은 시간축으로 큰 그림
- 컨센 경로 (38조→57.2조 +50%)
- 구체적 수치 예측 안 함
- 문장 끝난 후 빈 줄
...
1문단: 국면 정의 (짧게. 길어지면 빈 줄로 나누세요.)
- 첫 문장에 시장 국면 한마디 정의 (예: "3월 이후 조정 장세가 전환점을 맞이한 하루였습니다")
- 두 번째 문장에 그 배경 한 줄
...
- 화살표(→) 사용 금지. "A → B → C" 대신 "A가 B로 이어지며, C를 만들어내는 구조" 같은 자연어로
- "위험자산 선호 심리", "쌍끌이 수급", "반가운 신호" 같은 교과서/증권방송 용어 지양. 일상적이고 자연스러운 표현 사용
- 총 14~18문장
...
[레퍼런스 — 이 수준의 깊이와 흐름으로 작성하세요]
"전일 미국 증시는 장 초반 하르그섬 추가 공격과 이란 대화 중단 소식에 급락 출발했으나, 파키스탄의 중재로 2주 협상 기한 연장 기대감이 부각되면서 낙폭을 대부분 만회한 채 마감했습니다.

이후 트럼프가 호르무즈 해협 개방 조건 하 2주 휴전에 동의한다고 공식 발표했고, WTI는 장 마감 후 17% 급락하며 102달러대까지 내려왔습니다. 3월 이후 한 달 넘게 시장을 짓눌러온 지정학 리스크가 완화 국면에 접어든 것으로 보입니다.

오늘 한국 증시는 트럼프 휴전 선언이 가장 큰 변수입니다. WTI 급락은 에너지 비용 부담 완화, 인플레이션 우려 축소, 금리 인하 기대감 회복을 동시에 만들어내는 구조이며, 원달러 환율이 하락할 경우 외국인의 환차손 부담이 줄면서 수급 전환의 트리거가 될 수 있습니다.

전일 외국인이 순매수로 전환했는데, 최근 순매도 흐름에서 벗어나 이 순매수에 연속성이 생기는지가 핵심 관전 포인트입니다.

다만 2주 한시적 휴전이라는 점은 유의해야 하며, 매매 템포는 느리게 가져가는 것이 적절하지 않을까 싶습니다."

위 레퍼런스의 핵심 패턴:
- 장중 과정을 시간순으로 서술 (결론만 쓰지 않음)
- 인과관계 체인이 3단계 이상
- "3월 이후" 같은 시간축으로 큰 그림 제공
- 구체적 수치 예측("4%대 급등") 하지 않음
- 문장 끝난 후 빈 줄로 가독성 확보
...
1문단: 국면 정의 (짧게. 길어지면 빈 줄로 나누세요.)
- 첫 문장에 시장 국면 한마디 정의 (예: "3월 이후 조정 장세가 전환점을 맞이했습니다")
- 두 번째 문장에 핵심 동인 (예: "전쟁 휴전과 삼성전자 서프라이즈가 겹치면서~")

2문단: 미장 과정 (3~4문장)
- 장 초반 → 장중 → 마감 과정을 시간순으로
- 종목별 흐름을 자연스럽게 녹여서 서술

2문단: 한국 전망 (4~5문장)
- 한국 전망의 첫 문장은 반드시 매크로 변화(유가/환율/휴전 등)부터. 개별 종목 실적은 그 다음.
- 유가/환율/금리 변화가 한국 어떤 섹터에 영향 줄지 구체적으로
- 그 다음에 개별 실적(삼성전자 등) 언급
- 외국인 수급 변화 해석
- 오늘 핵심 이벤트/변수 (양면 시각)
