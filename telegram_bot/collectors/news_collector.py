...
3: 섹터·종목 — 매크로와 별도 문단. 주도 섹터 + 대표 종목 등락률 + 이유. 거래대금 상위 특이 종목 + 뉴스 매칭.
   실적 발표 종목은 컨센 경로 표시 (컨센 N조 대비 실제 N조, N% 상회/하회).
   전강후약이 나왔다면 "시장이 뭘 고민하고 있는지" 구조적 의문.
4: 수급 + 전망 — 외국인/기관 당일 수급 + 연속성 해석.
...
[데이터 활용]
- 채권금리(10Y-2Y 스프레드)가 제공되면 매크로 해석에 반영. 금리 변동이 증시에 미치는 영향을 인과관계에 포함.
- 심리지표(Fear & Greed, Put/Call)가 제공되면 시장 심리 판단 근거로 활용.
- 환율 데이터가 있으면 외국인 수급 해석에 연결.
- 급등락 종목은 반드시 뉴스 데이터에서 이유를 찾아 서술. 이유를 모르면 "사유 불분명"이라고 쓰지 말고 해당 종목을 언급하지 마세요.
- "밸류에이션"같은 전문용어를 쓸 때는 괄호 없이 자연어로 풀어서. 예: "주가 대비 기업가치 부담"
...
[구조]
1: 미장 과정 — 바로 내용 진입. 장 초반에서 장중, 마감까지 시간순.
2: 종목 — 강세 종목과 약세 종목 모두 서술. 강세만 나열하면 편향됩니다. 미장 과정과 같은 문단에 넣지 말 것.
3: 한국 체크포인트 — 첫 문장은 반드시 매크로(유가/환율/지정학)부터.
   인과관계 3단계 이상. "강세/약세로 출발할 것" 같은 방향성 단정 금지. 긍정 요인과 부정 요인을 병렬 서술.
   개별 종목 뉴스는 시가총액 상위(삼성전자, SK하이닉스 등) 위주로만. 중소형주 뉴스에 별도 문단을 할애하지 마세요. 언급하더라도 한 줄 이내.
4: 수급 + 큰 그림 — "~적절하지 않을까 싶습니다" 수준. 구체적 방향 예측 금지.
...
- 심리지표(Fear & Greed, Put/Call)가 제공되면 시장 심리 판단 근거로 활용.
- 급등락 종목은 반드시 뉴스 데이터에서 이유를 찾아 서술. 이유를 모르면 해당 종목을 언급하지 마세요.
- "밸류에이션"같은 전문용어는 자연어로 풀어서. 예: "주가 대비 기업가치 부담"
