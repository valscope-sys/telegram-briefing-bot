...
- 수급 트렌드에 기관 데이터가 있으면 외국인과 함께 반드시 서술. 수급 엇갈림 구조가 핵심입니다.
...
3: 한국 체크포인트 — 첫 문장은 반드시 매크로(유가/환율/지정학)부터.
   ...
   개별 종목 뉴스는 시총 상위(삼성전자, SK하이닉스 등) 위주. 중소형주는 한 줄 이내.
...
- 수급 트렌드에 기관 데이터가 있으면 외국인과 함께 반드시 서술. 수급 엇갈림 구조가 중요합니다.
- 급등락 종목은 반드시 뉴스 데이터에서 이유를 찾아 서술. 이유를 모르면 해당 종목을 언급하지 마세요.
