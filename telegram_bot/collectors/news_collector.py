...
4: 수급 + 큰 그림 — 외국인뿐 아니라 기관 수급도 반드시 포함. 수급 엇갈림 구조가 중요.
   컨텍스트는 배경 이해용. 매번 같은 문구(사이드카 N회, 서킷 N회 등)를 반복 인용하지 마세요. 큰 그림은 매일 다른 표현으로 쓰세요.
...
- "N거래일 연속"은 2일 이상만. "1거래일 연속"은 틀린 표현. 1일은 반드시 "전환"으로 쓰세요. 예: "순매도에서 순매수로 전환"
- ETF vs 종목 괴리 시 설명 필수
- 뉴스 데이터에 없는 정책명, 펀드명, 프로젝트명, 제품명을 만들어내지 마세요. 뉴스 목록에 있는 내용만 언급.
- 종목 급등락의 이유를 뉴스에서 못 찾으면 "전일 미국 반도체 강세 영향" 등 시장 전체 흐름으로 설명하세요. 관련 없는 뉴스를 억지로 붙이지 마세요.
...
- "N거래일 연속"은 2일 이상만. "1거래일 연속"은 틀린 표현. 1일은 반드시 "전환"으로 쓰세요.
- 뉴스 데이터에 없는 정책명, 펀드명, 프로젝트명, 제품명을 만들어내지 마세요. 뉴스 목록에 있는 내용만 언급.
- 종목 급등락의 이유를 뉴스에서 못 찾으면 "전일 미국 반도체 강세 영향" 등 시장 전체 흐름으로 설명하세요. 관련 없는 뉴스를 억지로 붙이지 마세요.
