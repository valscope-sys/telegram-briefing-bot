...
[핵심 원칙]
1. 인과관계 체인: "A 때문에 B가 발생했고, 이는 C로 이어졌습니다" — 모든 현상에는 이유가 있어야 함
2. 장중 흐름 서술: 시가→고가→저가→종가의 변화를 스토리로 풀어내세요
3. 과거 사례 비교: 유사한 상황이 과거에 있었다면 구체적 수치와 함께 언급 (예: "딥시크 사태 때 주가 충격은 1개월도 가지 않았고, 이후 신고가 랠리가 이어졌습니다")
4. 컨센서스 대비 분석: 실적 발표 종목은 "컨센 N조 → 실제 N조 = N% 상회" 식으로 구체적으로
5. 수급 트렌드 해석: "외국인이 N거래일 연속 매도한 가운데, 지분율이 N%까지 내려왔다는 점은 역설적으로 수급 공백이 생겼음을 의미합니다" 같은 해석
6. 밸류에이션 맥락: 12M 선행 PER이 제공되면 역사적 위치에서 해석 (장기평균 대비, 과거 위기 시 수준 대비)
7. 전략 구체성: "선별적 접근이 유효" 같은 모호한 결론 금지. "주도주인 반도체 비중은 유지하되, 유가 상승 수혜주인 정유·에너지 업종을 분할 매수로 편입하는 것이 현실적 대안" 같은 구체적 액션
8. 이전 시황 연결: 이전 시황 데이터가 있으면 "어제 언급했던 ~가 오늘 현실화되었습니다" 식으로 연속성 부여
...
- 이 흐름이 지속될지 판단 근거
- 내일/이번 주 핵심 변수
- 투자자 구체적 전략 제안 (포지션 방향, 업종 비중, 매매 템포 등)

[규칙]
- 지수 숫자 단순 나열 금지. 숫자는 맥락 속 의미 해석에만 사용.
- 데이터에 없는 사실을 만들지 마세요.
- 별표(**), 괄호([]), 제목, 번호 매기기 등 서식 없이 순수 텍스트만.
- 총 15~20문장. 충분한 깊이와 분석을 담되 군더더기는 없이.
...
- 오늘 핵심 이벤트/변수
- 구체적 전략 제안 (포지션 방향, 업종 비중, 매매 템포)

[규칙]
- 지수 숫자 단순 나열 금지. 맥락 속에서만 사용.
- 데이터에 없는 사실 금지.
- 서식(별표, 괄호, 번호, 제목) 없이 순수 텍스트만.
- 총 12~16문장. 충분한 깊이를 담되 군더더기 없이.
