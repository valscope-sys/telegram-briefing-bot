...
- HTS에서 볼 수 있는 숫자 나열은 가치가 없습니다. 투자자가 이 시황을 읽고 "그래서 내일 뭘 주목해야 하는지" 판단 재료를 얻어야 합니다.
- "뭐가 있었다" 나열이 아니라 "왜 그랬고, 그래서 뭐가 달라지는지" 해석을 쓰세요.
- 뉴스 섹션과 내용이 겹치면 안 됩니다. 뉴스 섹션은 개별 팩트, 시황은 종합 해석입니다.
...
- 뉴스 섹션 내용 재탕 금지. 뉴스에 나올 내용을 시황에서 앵무새처럼 반복하지 마세요. 시황은 종합 해석.
- 서식 없이 텍스트만
- 총 14~18문장
...
[핵심 원칙: "So what?" — 해석과 맥락이 가치다]
- 숫자 나열은 HTS에서 볼 수 있으므로 가치가 없습니다. "왜 그랬고, 그래서 오늘 뭘 봐야 하는지"를 쓰세요.
- 뉴스 섹션과 내용이 겹치면 안 됩니다. 시황은 종합 해석, 뉴스는 개별 팩트.
...
- 야간 프록시(KORU 등) 데이터가 있으면 "야간 프록시 기준 갭업/갭다운 예상" 수준으로 활용.
...
- "~예상됩니다" 1~2회 이내. "주목됩니다" 금지. 서식 없이 텍스트만.
- 총 12~16문장
