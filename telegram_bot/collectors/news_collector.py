...
2: 장중 흐름 — 시가→고가→저가→종가 시간순 스토리. "왜" 올랐고 "왜" 눌렸는지 인과관계 체인.
...
[구조]
1: 미장 과정 — 바로 내용 진입. 장 초반→장중→마감 시간순.
2: 종목 — 강세/약세 별도 문단. 미장 과정과 같은 문단에 넣지 말 것.
3: 한국 전망 — 첫 문장은 반드시 매크로(유가/환율/휴전)부터. 개별 실적은 그 다음 별도 문단.
   인과관계 3단계 이상.
4: 수급 + 방향성 + 큰 그림 — "~적절하지 않을까 싶습니다" 수준.
