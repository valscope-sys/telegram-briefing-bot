"""이브닝 브리핑 메시지 포맷터 (장후 16:30 — 메시지 1/N)

복원 (2026-05-27): 5/12 재작성에서 dict 키 미스매치로 깨졌던 KOSPI/수급/환율
라인 정확히 살림 (5c25436 commit 형식 그대로).

신고가 부분만 새 KRX universe 시스템 (sector_universe_*.json) 사용.
"""
import datetime
...
def format_evening_briefing(domestic_data, global_data, commentary, sector_data, highlow_data):
    ...
    # 🔺 52주 신고가 — KRX 지수/ETF 구성종목 단일 소스 역색인
    # 명세서 (2026-05-12 코드방): theme_groups 10개 묶음 폐기, sector_config 등록 순서.
    if highlow_data:
