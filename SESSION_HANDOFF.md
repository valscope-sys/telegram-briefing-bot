# 통합 세션 핸드오프 — 2026-04-29 (오후 update)

**모든 작업(브리핑봇 + 이슈봇)이 이 한 문서로 통합 운영됩니다.**

---

## 🆕 2026-04-29 핵심 변경 (이슈봇 대수술)

### 운영 모드: **C 모드 (풀 수동 / on-demand 전용)**
사용자 정책 (사비 운영 — 비용 최저 우선):
- 자동 폴링 **영구 제거** (Haiku 필터 호출 0)
- 카드 발송 X — 사용자 명시 요청 시에만
- 비용: 월 **$7~12** (시작 $90 대비 약 90% 절감)

`.env` 핵심:
```
ISSUE_BOT_AUTO_POLLING_ENABLED=false   # 영구
ISSUE_BOT_ENABLED=true                  # poller 유지 (DM 처리)
```

### 봇 명령어 (4개 + 자연어)
- `/help` — 안내
- `/card <URL>` 또는 URL만 — 메리츠 스타일 카드 생성
- `/dart [날짜] [시간범위] [기업명]` — DART 공시 조회 (자연어 가능)
- `/news [날짜] [시간범위] [키워드]` — RSS 헤드라인 조회 (자연어 가능)
- **자연어 입력**: rule + Haiku fallback (NLU 광범위 사전 200+ 표현)

### 핵심 구조 변경
- **DART corp_code 캐시** (~117,000 회사) — 30일 TTL 자동 재다운로드
- **DART URL fetch 보강** — iframe → KIND HTML 자동 추출
- **분기 자동 표시** — 발표 날짜로 1Q26·4Q25 등 추정 (잠정실적·정기보고서)
- **URL 코드 블록 표시** — `<code>https://...</code>` 길게 누르면 복사
- **면책 문구** — 미리보기 X, 채널 발송 시에만 자동 표시
- **다중 chat 지원** — `TELEGRAM_ALLOWED_CHAT_IDS` (그룹 추가/제거는 텔레 자체 기능)

### 등록된 Anthropic Routine
- **`market-impact-news-4h`** — 4시간 간격 (03/07/11/15/19/23 KST + 약 6분 지연)
- 작업: WebSearch로 한국 시장 파급력 큰 뉴스 5~7건 선별
- 첫 실행: **2026-04-29 19:05~19:11 KST**
- 결과 전달 방식: 이 세션 알림 또는 별도 task 기록 (첫 실행 후 확인 필요)
- Claude.ai 사이드바 → Scheduled → 관리

### 다음 작업 후보 (사용자 결정 대기)
1. ⭐ **4분기 추이 카드** (사용자가 가장 원했던 핵심)
   - "두산 실적" → 1Q26·4Q25·3Q25·2Q25 각각 카드 (분기별 컨센 비교)
   - 작업 ~250줄
   - 데이터: 네이버 증권 분기 추이 + DART 잠정실적
   - 카드 형식: 분기 사이 빈 줄, 매출·영업이익·순이익 + 컨센 차이%
2. **인라인 [📋 카드 만들기] 버튼** — URL 클릭 없이 한 번에 카드
   - 작업 ~100줄
   - DART/news 조회 결과 각 항목에 callback 버튼

### 카드 형식 사용자 정책 (확정)
```
1Q26 잠정실적 (2026-04-29)

매출액 : 8,512억 (예상치 : 8,200억 / +3.8%)
영업익 : 412억 (예상치 : 380억 / +8.4%)
순이익 : 285억 (예상치 : 270억 / +5.6%)

원문: https://dart.fss.or.kr/...
```
- 분기별로 빈 줄 구분
- 해석/시사점 섹션 X (사실만)
- 분기마다 원문 URL
- 면책 문구는 채널 발송 시에만 (미리보기엔 X)
- 1개 분기만 명시 → 1개 카드, 분기 미명시 → 4분기 카드

### NLU 광범위 사전 (rule + Haiku fallback)
13 카테고리 × 200+ 표현 등록:
- 요청 동사 / 존댓말 종결 / 반말 종결 / 모호 형용사 / 모호 명사
- 시간 부사 / 빈도·정도 / 추측 표현 / 연결 어미 / 종결 노이즈
- 감탄·구어 / 의문 보조 / 시제 보조 / 분기 복수표현
- 분기 패턴: "1Q26"/"1분기" → `#quarter:1Q26` 마커
- 보고서 키워드: "실적/자사주/M&A/Capex" 등 30+ → `#report:실적` 마커

### 운영 통계 / 작업 누적 (2026-04-29 하루)
- 총 commit: 20+ (자동 코드 -2,600줄, 신규 기능 +1,500줄)
- 핵심 commit:
  - `c574e3c` 자연어 NLU (Rule + Haiku fallback)
  - `88ed6bc` 다중 chat 지원
  - `9ebcc9f` DART corp_code + RSS 18개 + NLU 강화
  - `a504c7e` 광범위 한국어 사전 + 의도 분류 보강
